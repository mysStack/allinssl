...
export default defineComponent({
	...
	setup() {
		...
		const columns: DataTableColumns<DNSRecord> = [
			...
			{ title: '状态', key: 'status', width: 100, sorter: true, render: (record) => <NTag type={record.status === 'ENABLE' ? 'success' : 'warning'}>{record.status}</NTag> },
								...
								<NSpace vertical size="large">
									...
									{controller.snapshot.value ? (
										<NSpace vertical size="medium">
											...
											<NDataTable remote columns={columns} data={controller.visibleRecords.value} scrollX={1100} onUpdateSorter={(sorter: { columnKey?: string | number; order?: 'ascend' | 'descend' | false }) => { if (typeof sorter.columnKey === 'string' && sorter.order) controller.setRecordSort(sorter.columnKey as DNSRecordSortKey, sorter.order) }} />
										</NSpace>
									) : (
