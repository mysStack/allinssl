...
本设计以 `upstream/1.1.3`、提交 `73cbcb8a213d959e772fb8ab3120abb9efa476c4` 为上游基线；当前开发分支为 `custom`。上游同步前必须先执行：
...
- 所有可写类型必须有主机记录、记录值、`600` 至 `86400` 的整数 TTL 和有效线路；备注最长 `50` 个字符。
...
- 切换可编辑记录的类型时，使用原 `record_id` 调用 `UpdateDomainRecord`，例如将 CNAME 改为 A；不得删除后重建，也不得额外新增同名记录。
- 网络超时或结果不明确时不自动重试，返回“请刷新记录确认实际状态”，避免重复创建或误删。

### 解析线路与负载策略

`default` 是 AliDNS 的默认解析线路，不代表“轮询”。单条 A 或 AAAA 记录只返回其自身地址；同名、同类型、同线路的多条普通记录构成普通 DNS RR 集，递归解析器可能轮换其返回顺序。

AliDNS 的加权轮询是独立的 DNSLB 能力，需通过 `SetDNSSLBStatus` 开关记录集，并用 `UpdateDNSSLBWeight` 为记录设置 `1` 至 `100` 的权重。它与 SRV 记录自身的 `weight` 字段不同，不能复用。

页面仅在 `A`、`AAAA` 记录集编辑器中展示“记录值负载策略”：`轮询` 调用 `SetDNSSLBStatus(Open=false)`，`权重` 调用 `SetDNSSLBStatus(Open=true)` 后再为每个记录值调用 `UpdateDNSSLBWeight`。策略作用于同名、同类型、同线路的完整记录集，权重作用于其中的单个记录值；它与 SRV 自身的权重不同。

AliDNS 的读取结果会返回 CNAME 的 DNSLB 状态，因此列表会展示 CNAME 的策略和权重；但当前 SDK 的 `SetDNSSLBStatus` 写入接口仅声明支持 `A`、`AAAA`。CNAME 策略在本页面明确只读，不伪造写入能力。新增和更新均在厂商记录操作成功、重新读取完整 Zone 后设置 DNSLB 并再次刷新，任一步失败都提示用户刷新确认实际状态。

## 前端设计

页面沿用现有阿里云风格表格、完整读取、筛选和本地排序。移除“纳管预览”“DNSControl 状态”和 Preview 任务提示，新增如下操作：

- 顶部“添加记录”打开记录集编辑器；编辑任一记录时按“主机记录 + 类型 + 解析请求来源”聚合同一记录集。
- 编辑器顶部设置记录类型、主机记录、默认解析请求来源和 TTL；“记录值集合”逐行编辑记录值、A/AAAA DNSLB 权重、启停状态和备注，并支持新增、删除条目。
- 列表展示解析请求来源、负载策略、权重、TTL、状态和备注。表格内“编辑记录集”不会绕过保护记录规则；删除单条记录仍需二次确认。
- 多条记录值保存时复用已有单记录 API，按更新、新增、状态切换、删除的顺序逐条执行；AliDNS 不提供原子记录集事务，因此任何失败后前端必须重新读取 Zone，显示实际状态，禁止自动重试。
- 多值记录集禁止直接切换记录类型，避免在 CNAME 与其他类型之间制造冲突；将记录集缩减到一条后才能沿用 `UpdateDomainRecord` 切换类型。
...
- 前端测试：新增、按记录集聚合编辑、删除确认、启停、动态字段、只读按钮、成功刷新和失败提示。
