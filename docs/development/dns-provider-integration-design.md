...
页面仅在 `A`、`AAAA` 记录表单中展示“记录值负载策略”：`轮询` 调用 `SetDNSSLBStatus(Open=false)`，`权重` 调用 `SetDNSSLBStatus(Open=true)` 后再为当前记录调用 `UpdateDNSSLBWeight`。策略作用于同名、同类型、同线路的完整记录集，权重作用于其中的当前记录；不支持的 `CNAME`、TXT、MX、SRV 和 CAA 不显示该选项。新增和更新均在厂商记录操作成功、重新读取完整 Zone 后执行 DNSLB 设置并再次刷新，任一步失败都提示用户刷新确认实际状态。
...
- 表单按记录类型动态显示 MX、SRV、CAA 的专属字段，并允许输入 AliDNS 解析线路，默认值为 `default`；编辑时可切换记录类型，并明确提示 A/AAAA 必须填写 IP 地址。
